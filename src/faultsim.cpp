      ...
      while (!wlist_faulty.empty()) {
        ...
        if(w->is_output()) {
          for(i = 0; i < num_of_fault; ++i) {
            if(simulated_fault_list[i]->detect) continue;
            const int v1 = w->wire_value1 & Mask[i];
            const int v2 = w->wire_value2 & Mask[i];
            if(v1 == Unknown[i] || v2 == Unknown[i]) continue;
            if(v1 != v2) {
...
void ATPG::inject_fault_value(const wptr faulty_wire, const int &bit_position, const int &fault_type) {
   ...
   */
  if(fault_type == STUCK0) faulty_wire->wire_value2 &= ~Mask[bit_position];
  if(fault_type == STUCK1) faulty_wire->wire_value2 |=  Mask[bit_position];
